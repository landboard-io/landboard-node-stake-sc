...
upgrade() {
    erdpy --verbose contract upgrade ${ADDRESS} --project=${PROJECT} --recall-nonce --pem=${WALLET} --gas-limit=100000000 --send --outfile="upgrade.json" --proxy=${PROXY} --chain=${CHAIN_ID} \
    --arguments ${TOKEN_ID_HEX} ${TOKEN_ID_HEX} ${REFERRRAL_ACTIVATION_AMOUNT} ${APY_INCREASE_PER_REFERRAL} ${MAX_APY_INCREASE_BY_REFERRAL} ${PROMO_INCREASE_APY} ${TOTAL_REFERRAL_COUNT_LIMIT}
}
...
addStakeTypes() {
    ...
    --arguments 6000 60 10000000000000000 3000 5000 30 30 100000000000000000000 5000 10000 \
...
stake() {
    ...
    --recall-nonce --pem=${WALLET} \
    --gas-limit=10000000 \
    --data="ESDTTransfer@${TOKEN_ID_ONLY_HEX}@016345785d8a0000@${STAKE_ONLY_HEX}@01@${CALLER2_ADDRESS_ONLY_HEX}" \
    --send --proxy=${PROXY} --chain=${CHAIN_ID}
}
...
getClaimableAndReward() {
    erdpy --verbose contract query ${ADDRESS} --proxy=${PROXY} --function="getClaimableAndReward" --arguments ${CALLER_ADDRESS_HEX} 1
}
